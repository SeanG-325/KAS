    ...
    with open(out_file, 'wb') as writer:
        ...
        for i, new_line in enumerate(lines):
            ...
            if i % 4 == 3:
                ...
                tf_example.features.feature['article_a'].bytes_list.value.extend([bytes(article_a, encoding='utf-8')])
                tf_example.features.feature['article_aa'].bytes_list.value.extend([bytes(article_aa, encoding='utf-8')])
                ...
                if makevocab:
                    ...
                    art_tokens_2 = article_a.split(' ')
                    art_tokens_aa = article_aa.split(' ')
                    abs_tokens = abstract.split(' ')
                    abs_tokens = [t for t in abs_tokens if
                                  t not in [SENTENCE_START, SENTENCE_END]]  # 从词典中删除这些符号
                    tokens = art_tokens + art_tokens_2 + art_tokens_aa + abs_tokens
                    tokens = [t.strip() for t in tokens]  # 清楚句子开头结尾的空字符
                    tokens = [t for t in tokens if t != "" and not t.isdecimal()]  # 删除空行
